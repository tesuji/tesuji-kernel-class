...
static long firewall_ioctl(struct file *file, unsigned int cmd,
                           unsigned long arg);
static uint32_t firewall_inbound_hook(void *priv, struct sk_buff *skb,
                                      const struct nf_hook_state *state);
static uint32_t firewall_outbound_hook(void *priv, struct sk_buff *skb,
                                       const struct nf_hook_state *state);
static uint32_t process_rule(struct sk_buff *skb, rule_t *rule, uint8_t type,
                             int i);
static long firewall_add_rule(user_rule_t user_rule, rule_t **firewall_rules,
                              uint8_t idx);
static long firewall_delete_rule(user_rule_t user_rule, rule_t **firewall_rules,
                                 uint8_t idx);
static long firewall_edit_rule(user_rule_t user_rule, rule_t **firewall_rules,
                               uint8_t idx);
static long firewall_show_rule(user_rule_t user_rule, rule_t **firewall_rules,
                               uint8_t idx);
static long firewall_dup_rule(user_rule_t user_rule, rule_t **firewall_rules,
                              uint8_t idx);

static struct miscdevice firewall_device;
static struct file_operations firewall_fops = {.unlocked_ioctl =
                                                   firewall_ioctl};

static struct nf_hook_ops in_hook = {.hook = firewall_inbound_hook,
                                     .hooknum = NF_INET_PRE_ROUTING,
                                     .pf = PF_INET,
                                     .priority = NF_IP_PRI_FIRST};

static struct nf_hook_ops out_hook = {.hook = firewall_outbound_hook,
...
static long firewall_ioctl(struct file *file, unsigned int cmd,
  ...
  if (copy_from_user((void *)&user_rule, (void *)arg, sizeof(user_rule_t))) {
    printk(KERN_INFO
           "[Firewall::Error] firewall_ioctl() cannot copy user request!\n");
  ...
  firewall_rules =
      (user_rule.type == INBOUND) ? firewall_rules_in : firewall_rules_out;
...
static long firewall_add_rule(user_rule_t user_rule, rule_t **firewall_rules,
  ...
  if (in4_pton(user_rule.netmask, strnlen(user_rule.netmask, 16),
               (u8 *)&(firewall_rules[idx]->netmask), -1, NULL) == 0) {
    printk(KERN_ERR
...
static long firewall_dup_rule(user_rule_t user_rule, rule_t **firewall_rules,
  ...
  if (firewall_rules[idx]->is_duplicated) {
    printk(KERN_INFO
           "[Firewall::Info] firewall_dup_rule() rule already duplicated "
           "before!\n");
    return ERROR;
  }
...
static uint32_t process_rule(struct sk_buff *skb, rule_t *rule, uint8_t type,
  ...
  if (type == INBOUND) {
    if ((rule->ip & rule->netmask) != (iph->saddr & rule->netmask)) {
      printk(KERN_INFO
             "[Firewall::Error] Rule[%d], ip->saddr doesn't belong to the "
             "provided subnet, skipping!\n",
             i);
      return SKIP;
    }
  } else {
    if ((rule->ip & rule->netmask) != (iph->daddr & rule->netmask)) {
      printk(KERN_INFO
             "[Firewall::Error] Rule[%d], ip->daddr doesn't belong to the "
             "provided subnet, skipping!\n",
             i);
      return SKIP;
    }
  ...
  if ((rule->proto == IPPROTO_TCP) && (iph->protocol == IPPROTO_TCP)) {
    ...
    if ((rule->port != 0) && (rule->port != tcph->dest)) {
      printk(KERN_INFO
             "[Firewall::Error] Rule[%d], rule->port (%d) != tcph->dest (%d), "
             "skipping!\n",
    ...
    if ((rule->port != 0) && (rule->port != udph->dest)) {
      printk(KERN_INFO
             "[Firewall::Error] Rule[%d], rule->port (%d) != udph->dest (%d), "
             "skipping!\n",
